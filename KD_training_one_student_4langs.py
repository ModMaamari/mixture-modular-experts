...
with open(f'tokenized_datasets/imbalanced_tokenized_train_datasets.pkl', 'rb') as file:
    shuffle_seed = 2024
    np.random.seed(shuffle_seed)
    tokenized_train_datasets = pickle.load(file)
    # Merge and Shuffle
    # Convert the dictionary to a list of datasets
    dataset_list = list(tokenized_train_datasets.values())

    # Concatenate the datasets
    tokenized_train_datasets = concatenate_datasets(dataset_list)

    # Shuffle the merged dataset
    tokenized_train_datasets = tokenized_train_datasets.shuffle(seed=shuffle_seed)
    # tokenized_train_datasets = {key: dataset.shuffle() for key, dataset in tokenized_train_datasets.items()}
    if drop_language_feature:
        tokenized_train_datasets = tokenized_train_datasets.dataset.remove_columns('language')
...
#     train_balanced_ds = BalancedLanguageBatchDataset(train_datasets_separated, batch_size, device=device, random_batches=False)
...
output_dir = f"./KD_one_stdnt_4langs_{int(student_model_params_millions)}M_exp_results_{datetime.now().strftime('%Y-%m-%d %H:%M')}"
epochs=3
train_batch_size = 32
eval_batch_size = 64
len_train_data = len(tokenized_train_datasets) #sum([x.num_rows for x in tokenized_train_datasets.values()])
...
wandb.save('KD_training_one_student_4langs.py')
