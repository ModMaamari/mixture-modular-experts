...
```
python src/transformers/models/modumix/convert_modumix_weights_to_hf.py \
    --input_dir /path/to/downloaded/modumix/weights --model_size 7B --output_dir /output/path
```

Thereafter, models can be loaded via:

```py
from transformers import ModumixForCausalLM

model = ModumixForCausalLM.from_pretrained("/output/path")
```
...
def write_model(model_path, input_base_path, model_size, safe_serialization=True):
    ...
    print("Loading the checkpoint in a Modumix model.")
    with torch.device("meta"):
        model = ModumixForCausalLM(config)
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        help="Location of Modumix weights, which contains tokenizer.model and model folders",
        required=True,
    )
    parser.add_argument(
        "--model_size",
        choices=["7B"],
        help="'f' models correspond to the finetuned versions, and are specific to the Modumix official release. For more details on Modumix",
        default="7B",
    )
